...
public class SpamCampaignSet {
	...
	SpamCampaignSet(int n_max, int K, int M, String l_file, 
			double[] tau, double r){
		this.n_max = n_max;
		this.K = K;
		this.M = M;
 ...
 */
int[] MaxSubUnion(int x, HashSet<Integer> vn, HashSet<Integer> vnComp){
	int[] ret = new int[x];
 ...
 * vnComp: the set of selected socialbots
 */
int[] MinSubUnion(int x, HashSet<Integer> vn, HashSet<Integer> vnComp){
	int[] ret = new int[x];
...
public static void main(String[] args){
	...
	for (int i = 1; i <= n_num; i ++){
		for (int j = 1; j <= s1_num; j ++){
			...
			SpamCampaignSet spam = new SpamCampaignSet(n, K, M, l_file, tau, r);
